...
def retrieve_relevant_chunks(user_input: str, collection_name: str = "document_embeddings", top_k: int = 5) -> List[Dict]:
    """根据用户输入从 Qdrant 中检索相关的文本块"""
    task = Task.init(project_name="Featurization_Pipeline", task_name="User Input Retrieval")
    ...
    try:
        ...
        with torch.no_grad():
            ...
            print("Generated embedding:", user_embedding[:5])  # 打印嵌入前5个值
...
def generate_answer(prompt: str) -> str:
    """使用 smollm 135 生成回答"""
    task = Task.init(project_name="Featurization_Pipeline", task_name="Response Generation")
    logger = task.get_logger()
...
if __name__ == "__main__":
    ...
    retrieved_chunks = retrieve_relevant_chunks(user_input_2)
    if not retrieved_chunks:
        print("No relevant documents found.")
    else:
        prompt = generate_prompt(user_input, retrieved_chunks)
